...
WITH (SYSTEM_VERSIONING = ON (HISTORY_TABLE=[history].[distribution_lists], DATA_CONSISTENCY_CHECK=ON));
GO

ALTER TABLE [dbo].[distribution_lists]
    ADD CONSTRAINT [unq_distribution_lists] UNIQUE NONCLUSTERED ([name] ASC);
GO

ALTER TABLE [dbo].[distribution_lists]
    ADD CONSTRAINT [pk_distribution_lists] PRIMARY KEY CLUSTERED ([id] ASC);
GO
