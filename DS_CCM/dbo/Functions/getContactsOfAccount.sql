create function [dbo].[getContactsOfAccount](@id int)
returns nvarchar(max)
as
BEGIN
	declare @json_text nvarchar(max);
    with con as(
        select c.id,c.person,isnull(c.billing_account,csa.customer_billing_account) billing_account,c.sage_account,
        (select
            c.id,
        ...
        WHERE
            isnull(c.billing_account,csa.customer_billing_account)=@id),
        a(jsn_txt) as(
        select 
            JSON_QUERY((select '['+string_agg(jsn,',')+']' from con where sage_account is null)) as contacts,
            (select
                csa.id,
            ...
            where
                csa.id in (select sage_account from con)
