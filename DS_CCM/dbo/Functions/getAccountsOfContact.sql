CREATE function [dbo].[getAccountsOfContact](@id int)
returns nvarchar(max)
as
BEGIN
	declare @json_text nvarchar(max);
	with con as 
        (select c_out.id,isnull(c_out.billing_account,csa.customer_billing_account) billing_account,c_out.sage_account,(
            select 
                c_in.id,
        ...
        where person =@id and location is null and (c_out.billing_account is not null or c_out.sage_account is not null))
        ,a(jsn_txt) as
        (
            select 
                cba.id,
            ...
            where
                    cba.id in (select billing_account from con) 
            FOR JSON PATH, INCLUDE_NULL_VALUES
        )
